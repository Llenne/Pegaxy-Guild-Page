...
interface LeaderboardTopEarningGuildsData {
    address: string;
    index: number;
    rank: number;
    vis: number;
...
interface LeaderboardTopEarningGuildsProps {
    data: Array<LeaderboardTopEarningGuildsData>;
    loading: boolean;
}

export const LeaderboardTopEarningGuilds: FC<LeaderboardTopEarningGuildsProps> = ({ data, loading }) => {
    return (
        <Box>
            <Card sx={{ boxShadow: 10 }}>
                <CardHeader
                    title={"Top Earning Owners (All-Time)"}
                    // action={(
                    ...
                    loading ? (
                        ...
                        <Scrollbar>
                            <Table>
                                ...
                                <TableBody>
                                    {
                                        data.map((row: LeaderboardTopEarningGuildsData) => (
                                            <TableRow
                                                key={row.index}
                                                sx={{
                                                    '&:last-child td': {
